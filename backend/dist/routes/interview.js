...
interviewRouter.get('/:id/start', authMiddleware, async (req, res, next) => {
    try {
        ...
        const voiceId = req.query.voiceId || undefined;
        const greetingResult = await llmCallJSON(user, system);
...
interviewRouter.post('/:id/answer', authMiddleware, upload.single('audio'), async (req, res, next) => {
    try {
        ...
        const transcriptOverride = req.body.transcriptOverride;
        ...
        const [sttResult, context, countResult] = await Promise.all([
            ...
            buildQuestionContext(interviewId, (question.topic ?? question.question_text)),
        ...
        const evaluation = await llmCallJSON(eUser, eSys, { temperature: 0.3 });
        ...
        supabase.from('evaluations').insert({
            ...
            if (error)
                console.error('[Answer] Eval insert error:', error.message);
        });
        ...
        if (!isComplete) {
            ...
            const nextQuestionData = await llmCallJSON(qUser, qSys);
            nextQuestionId = uuid();
            ...
            supabase.from('questions').insert({
                ...
                if (error)
                    console.error('[Answer] Question insert error:', error.message);
            });
