function clientStatusAndMessage(err) {
    const raw = err.message ?? 'Internal Server Error';
    const code = err.code;
    if (raw === 'Malformed part header' || raw.includes('Unexpected end of form')) {
        return {
            statusCode: 400,
            message: 'Invalid multipart upload. In API clients, do not set Content-Type yourself for form-data (the tool must add boundary=...). Re-attach the file and ensure the field name is "resume".',
        };
    }
